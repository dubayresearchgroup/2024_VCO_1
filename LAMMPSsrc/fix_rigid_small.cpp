...
void FixRigidSmall::apply_langevin_thermostat()
{
  ...
  for (int ibody = 0; ibody < nlocal_body; ibody++) {
    ...
    gamma2 = sqrt(body[ibody].mass) * tsqrt *
      sqrt(2.0*boltz/t_period/dt/mvv2e) / ftm2v;

    double a = gamma2*(random->gaussian());
    double b = gamma2*(random->gaussian());
    double c = gamma2*(random->gaussian());

    // std::ofstream myfile;

    // myfile.open ("Random_Noises_Gauss_xForce.txt", std::ios_base::app);   
    // myfile << a << "\n"; 
    // myfile.close();

    // myfile.open ("Random_Noises_Gauss_yForce.txt", std::ios_base::app);   
    // myfile << b << "\n"; 
    // myfile.close();

    langextra[ibody][0] = gamma1*vcm[0] + a;
    langextra[ibody][1] = gamma1*vcm[1] + b;
    langextra[ibody][2] = gamma1*vcm[2] + c;

    gamma1 = -1.0 / t_period / ftm2v;
    gamma2 = tsqrt * sqrt(2.0*boltz/t_period/dt/mvv2e) / ftm2v;
    double d = gamma2*(random->gaussian());
    double e = gamma2*(random->gaussian());
    ...
    langextra[ibody][5] = inertia[2]*gamma1*omega[2] +
      sqrt(inertia[2])*gamma2*(random->gaussian());

    // myfile.open ("Random_Noises_Gauss_xInertia.txt", std::ios_base::app);   
    // myfile << a << "\n"; 
    // myfile.close();

    // myfile.open ("Random_Noises_Gauss_yInertia.txt", std::ios_base::app);   
    // myfile << b << "\n"; 
    // myfile.close();
